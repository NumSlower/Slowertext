...
void Buffer::insert_char(int x, int y, char c) {
    ...
    if (x < 0 || x > static_cast<int>(lines[y].length())) x = static_cast<int>(lines[y].length());
...
void Buffer::delete_char(int x, int y) {
    if (y < 0 || y >= static_cast<int>(lines.size())) return;
    if (x < 0 || x >= static_cast<int>(lines[y].length())) return;
    lines[y].erase(x, 1);
    modified = true;
}
...
void Buffer::set_line(int y, const std::string& line) {
    if (y < 0) return;
    while (y >= static_cast<int>(lines.size())) lines.push_back("");
    lines[y] = line;
    modified = true;
}

bool Buffer::is_modified() const { return modified; }
void Buffer::set_modified(bool mod) { modified = mod; }
...
const std::vector<std::string>& Buffer::get_lines() const { return lines; }