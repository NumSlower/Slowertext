...
static void init_editor() {
    ...
    // Config loading happens here; it may override mode, colours, etc.
    ConfigManager::load_config(editor_config);

    if (editor_config.debug_mode) {
        std::cerr << "Debug: Configuration loaded\n"
                  << "Debug: tab_width           = " << editor_config.tab_width           << '\n'
                  << "Debug: show_line_numbers   = " << editor_config.show_line_numbers   << '\n'
                  << "Debug: auto_indent         = " << editor_config.auto_indent         << '\n'
                  << "Debug: syntax_highlighting = " << editor_config.syntax_highlighting << '\n'
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        if (argc >= 2) {
            ...
            set_status_message("SlowerText  |  Tab width: " +
                               std::to_string(editor_config.tab_width) + " spaces");
        }
