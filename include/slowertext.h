...
// 8-color named codes (legacy / fallback)
...
// ── Editor modes ─────────────────────────────────────────────────────────────
enum EditorMode { INSERT_MODE, COMMAND_MODE };

// ── Color target (for live :color command) ───────────────────────────────────
enum ColorTarget {
    COLOR_TARGET_TEXT,
    COLOR_TARGET_BACKGROUND,
    COLOR_TARGET_STATUS_BAR,
    COLOR_TARGET_STATUS_TEXT,
    COLOR_TARGET_COMMENT,
    COLOR_TARGET_LINE_NUMBER,
    COLOR_TARGET_CURRENT_LINE,
    COLOR_TARGET_UNKNOWN
};
...
struct EditorConfig {
    ...
    // Dirty flag: set on any state change; cleared after each redraw
    bool        needs_redraw;
    ...
    // Colors — each accepts: named | colorN (0-255) | #rrggbb
    ...
    int         buffer_size;
    int         refresh_rate;   // target FPS; used for select() timeout
...
public:
    ...
    static void        parse_config_line(const std::string& line,
                                          std::map<std::string,std::string>& values);
    static void        apply_config_values(EditorConfig& config,
...
public:
    static int  read_key();
    // Returns true if a keypress was processed; false on timeout (no input ready)
    static bool process_keypress(EditorConfig& config, Buffer& buffer);
    static void process_command(EditorConfig& config, Buffer& buffer,
...
// ── Color resolution (used by renderer + command processor) ──────────────────
// Resolve a color spec to an ANSI SGR foreground escape string.
// Accepts: named ("white","red",...) | "colorN" (N 0-255) | "#rrggbb"
std::string resolve_fg(const std::string& spec);

// Resolve a color spec to an ANSI SGR background escape string.
std::string resolve_bg(const std::string& spec);

// Map a target name string to a ColorTarget enum value.
ColorTarget parse_color_target(const std::string& name);

// ── twrite helper ─────────────────────────────────────────────────────────────
#include <cstddef>
inline void twrite(int fd, const void* buf, std::size_t n) {
    if (write(fd, buf, n)) { /* intentionally discarded */ }
}
